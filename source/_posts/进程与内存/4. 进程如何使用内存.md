...
RSS/RES在反映物理内存上也不是最准确的，因为这两个指标会将多进程共享的内存页算成私有的，指标的结果可能比真正占用内存大。真正反应真实物理内存占用的指标是`PSS`，可以使用以下命令查看，可以看到PSS的占用比RSS小不少。
```shell
cat /proc/{pid}/smaps_rollup
5642a66bc000-7ffe9b3cf000 ---p 00000000 00:00 0                          [rollup]
Rss:                8716 kB
Pss:                4311 kB
```
